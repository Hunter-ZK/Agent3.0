...
from sql_pilot_engine.reporting.renderers import (
    render_json,
    render_markdown,
    render_text,
)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Agent3.0 Trusted SQL CLI for MaxCompute / DataWorks"
    )
    ...
    parser.add_argument(
        "--output",
        default=None,
        help="审查报告输出路径。不传则打印到控制台。",
    )
    parser.add_argument(
        "--mode",
        choices=["debug", "prod", "backfill"],
        default="prod",
        help="运行模式。",
    )
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="只运行指定分类规则，可重复传入。",
    )
    parser.add_argument(
        "--enable-metadata",
        action="store_true",
        help="启用 synthetic Mock Metadata 检查。",
    )
    parser.add_argument(
        "--enable-llm",
        action="store_true",
...
def _build_engine(args: argparse.Namespace):
    need_llm_client = (
        ...
        or (
            args.fix_sql
            and args.fix_provider == "llm"
        )
    )

    llm_client = (
        _build_llm_client(provider=args.llm_provider)
        if need_llm_client
        else None
    )
...
def main() -> None:
    ...
    if not response.success or response.raw_result is None:
        raise RuntimeError(
            ...
            or "SQL Review Engine 执行失败。"
        )

    result = response.raw_result
    rendered_result = render_result_by_format(
    ...
    if args.fix_sql and result.fixed_sql_result is not None:
        fixed_output = (
            args.fixed_output
            or str(sql_path.with_suffix(".fixed.sql"))
        )
        fixed_path = Path(fixed_output)
        write_text_file(
            fixed_path,
            result.fixed_sql_result.fixed_sql,
        )
