...
from sql_pilot_engine.generation.prompts import render_query_context
...
def test_planner_can_request_clarification():
    planner = QueryPlanner(model=NeedClarificationModel())
    ...
    assert result.reason == "当前上下文不足以可靠生成SQL。"
...
def test_session_context_is_rendered():
    ...
    assert "同比和环比都计算增长率" in rendered
