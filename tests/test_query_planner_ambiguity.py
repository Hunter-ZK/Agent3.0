...
SEMANTIC_CONTEXT = (
    ...
    "METRIC tech_loan_balance synonyms=贷款余额\n\n"
    "TABLE dwd_hd_201_cldwdk\n"
    "METRIC green_loan_balance synonyms=贷款余额"
)
...
def test_planner_can_clarify_ambiguous_business_subject():
    planner = QueryPlanner(model=AmbiguousLoanPlannerModel())
    ...
    assert isinstance(result, PlanningClarification)
    assert result.missing_context == ("贷款业务主题",)
    assert "科技贷款余额" in result.clarification_question
    assert "绿色贷款余额" in result.clarification_question


class ResolvedLoanPlannerModel:
    def generate(self, prompt: str) -> str:
        assert "当前任务讨论绿色贷款" in prompt
        return """
        {
          "status": "ready",
          "plan": {
...
def test_session_context_can_resolve_business_ambiguity():
    planner = QueryPlanner(model=ResolvedLoanPlannerModel())
    ...
    assert result.tables == ("dwd_hd_201_cldwdk",)
    assert result.metrics == ("green_loan_balance",)
